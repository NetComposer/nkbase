    ...
    {setup, spawn,
        ...
        fun({ok, _PathM, LevelM, _File2, LevelS}) ->
            [
                ...
                {timeout, 60, fun() ->
                    dist_store({leveldb, LevelM}, {leveldb, LevelS}) end},
...
all_ets() ->
    MemM = ets:new(backend_test_m, [named_table, ordered_set, public]),
    MemS = ets:new(backend_test_s, [named_table, ordered_set, public]),
    put_remove({ets, MemM}),
    dist_store({ets, MemM}, {ets, MemS}),
    bulk_insert({ets, MemM}),
    scan_domains({ets, MemM}),
    scan_classes({ets, MemM}),
    scan_keys({ets, MemM}),
    search1({ets, MemM}),
    search2({ets, MemM}),
    bulk_remove({ets, MemM}),
    ok.
...
all_leveldb() ->
    ...
    {ok, LevelM} = eleveldb:open(PathM, Opts),
    {ok, LevelS} = eleveldb:open(PathS, Opts),
    put_remove({leveldb, LevelM}),
    dist_store({leveldb, LevelM}, {leveldb, LevelS}),
    bulk_insert({leveldb, LevelM}),
    scan_domains({leveldb, LevelM}),
    scan_classes({leveldb, LevelM}),
    scan_keys({leveldb, LevelM}),
    search1({leveldb, LevelM}),
    search2({leveldb, LevelM}),
    bulk_remove({leveldb, LevelM}),
    ok.
